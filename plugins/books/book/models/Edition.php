...
class Edition extends Model implements ProductInterface
{
    ...
    {
        $allowed = [BookStatus::WORKING, BookStatus::COMPLETE];
        if (in_array($this->getOriginal('status'), $allowed)) {
            return true;
        }
    ...
    {
        return in_array($this->getOriginal('status'), [BookStatus::HIDDEN, BookStatus::COMPLETE])
